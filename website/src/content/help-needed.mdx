...
If so, we recommend checking first for existing [GitHub Issues](https://github.com/commercetools/merchant-center-application-kit/issues) about similar problems. Otherwise, we suggest opening a new [GitHub Issue](https://github.com/commercetools/merchant-center-application-kit/issues/new/choose) describing your problem.
...
Chrome has a setting option that prevents saving cookies, which makes it impossible to have a valid session when developing the Custom Applications locally. The option **must be turned on**.
