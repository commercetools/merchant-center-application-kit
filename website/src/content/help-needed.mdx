...
Merchant Center platform has only one environment where Custom Applications are installed to, but a similar goal can be achieved by setting up the Custom Application configuration so it uses a different [`entryPointUriPath`](/api-reference/application-config#entrypointuripath) configuration property per its installation deployments.

<Info>

More information about how to set environment specific Custom Applications cofiguration properties can be found in [this section](/api-reference/application-config#environment-variable-references).
...
However, there is an important update necessary to be applied to the Custom Application source code.
