...
This generally can be achieved using preview deployments, where you can inspect different versions of the Custom Application. As of now, the Merchant Center does not support this feature yet.
...
However, there is also another needed update to be applied to the Custom Application source code related to user permissions.
...
Nevertheless is a caveat though since the `entryPointUriPath` is not a static value in this scenario so we would need to tailor the source code to calculate it accordingly as we showcase in the next example.
