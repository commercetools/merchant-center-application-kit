...
The Merchant Center is translated in several languages like `en`, `de`, `es`, etc. The **default language** is always `en`.
...
As a first step, we recommend to define source messages in `messages.js` file next to your React component.
...
Source messages are defined as named objects by [`react-intl`](https://github.com/formatjs/react-intl), a library with React abstractions on top of the [ECMAScript Internationalization API](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl).

<Info>

We recommend to get familiar with the [ICU Message syntax](https://formatjs.io/docs/core-concepts/icu-syntax) to take advantage of the different syntax features.

</Info>

The source messages can then be used in your React components.<br />
There are different ways of consuming Intl messages.
...
Most of the UI-Kit components support formatting messages via the prop `intlMessage`. You can simply pass your "raw" message and the component takes care of rendering the message.
