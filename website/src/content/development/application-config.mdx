...
A Custom Application requires certain information to be able to work, for example which commercetools APIs should be used in which cloud region.
...
The file needs to be located in the project root folder and is automatically loaded by the application kit packages.

<Info>

The application config is the new recommended replacement of the `env.json` and `headers.json` files. Read the [release notes on how to migrate to the new config](/releases/2020-07-16-introducing-a-new-and-simpler-application-config).
...
The configuration file tries to require only the minimal necessary information as most of the data is automatically inferred.
...
You can also inspect the [JSON schema](https://unpkg.com/@commercetools-frontend/application-config/schema.json) to see all the available properties.

</Info>

Here is the list of properties and how they should be used:

### `entryPointUriPath`

The `entryPointUriPath` refers to the unique route path of the Custom Application. This is the identifier that the Merchant Center Proxy uses to match the HTTP request and forward it to the Custom Application. This value also needs to be used in the application client side routes.
...
The `env` object represents different environments (for example `production`). The environment used depends on the environment variable `MC_APP_ENV`. If `MC_APP_ENV` isn't set then `NODE_ENV` will be used, if it's not set then it defaults to `development`.
...
The `url` is the full URL value where the Custom Application runs in production. This value is also used to configure the Content-Security-Policy HTTP headers to allow the browser to request data from the application domain.
...
References are specified with a special expansion-like syntax `${}` and the prefix key `env:`: `${env:VALUE}`.
