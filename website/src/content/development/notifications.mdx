...
Either way, a user should be informed what was the end result of the action.

The Custom Application built-in notification system provides a way of showing notifications by dispatching Redux actions throught bound action creators or React hooks exposed from the [`@commercetools-frontend/actions-global`](/api-reference/commercetools-frontend-actions-global) package.
...
For instance, to provide users with the best experience we encourage to map errors related to particular form fields to field error messages (rather than dispatching a notification).
