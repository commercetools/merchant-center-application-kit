...
In the previous section, we learned how to develop forms. In this section we look at how to communicate feedback to users using notifications, in particular in response to user actions.
...
import { 
  DOMAINS, 
  NOTIFICATION_KINDS_SIDE, // `error`, `warning`, `info`, `success`
  NOTIFICATION_KINDS_PAGE // `error`, `warning`, `info`, `success`, `unexpected-error`, `api-error`
...
As a rule of thumb, `error` notifications should be diplayed in the `page` domain rather than the `side` domain.
