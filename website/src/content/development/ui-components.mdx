...
There are two main variants of the `<Spacings>` components: `Stack` and `Inline`.<br />
...
By default, the `Stack` component has a default `scale` of `s`. The `scale` determines the "space" between each children.<br />
...
There are several variants of the `<Text>` components: `Headline`, `Subheadline`, `Body`, and `Detail`.<br />
You can think about these in the following way:
