...
The commercetools platform and the Merchant Center have first-class support for GraphQL and we **strongly recommend to build your Custom Application using GraphQL** wherever possible.
...
In our case we simply want to print out the `error.graphQLErrors`. We can attempt to do that by implementing a helper function like this:
...
We would then use our connector hook in the `Channels` component instead of using the Apollo Client hooks directly. Therefore, there is less code in the React component as most of the logic and configuration is abstracted away in the connector hook.
