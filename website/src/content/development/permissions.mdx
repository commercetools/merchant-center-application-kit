...
By default the Custom Application automatically checks for "view" permission in the main application route `/:projectKey/:entryPointUriPath`, based on the `entryPointUriPath`. This is useful to enforce that the Custom Application can only be accessed with the correct permissions.

For example, if the `entryPointUriPath` is set to `avengers`, the Custom Application checks that the user has the `ViewAvengers` permission.
