...
One last step to see first UI change is to update the messages of view-one component, replace `<project-root>/src/components/view-one/messages.js` code with following code.
...
Add following code to `update-container-form.js`. to handle loading, error and dataTable if it's success.
```js
import React from 'react';
...
```
Getting Custom Objects for given container is queried using `get-custom-objects-by-container.graphql` query, `useQuery` from `@apollo/client` and target platfrom from `@commercetools-frontend/constants`.
...
export default ViewOne;
```
If it's successful, you should be something like below.
...
```
Add following code to `create-or-update-custom-objects-form.js`.
```js
import React from 'react';
import TextInput from '@commercetools-uikit/text-input';
import PrimaryButton from '@commercetools-uikit/primary-button';
