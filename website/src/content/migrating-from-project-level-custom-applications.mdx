...
The [new release](/releases/2022-01-25-custom-applications-v21) of Custom Applications is what we call and refer to as **Org-level Custom Applications**.
...
* **Project-level Custom Applications** refers to the legacy functionality up until now. This is still available in maintenance mode during the [migration period](#migration-period) and will be removed afterwards.

<Info>

The documentation for Project-level Custom Applications [remains available](https://docs.commercetools.com/custom-applications/_legacy]) for the [migration period](#migration-period).
...
What if this mapping of User Permissions and OAuth Scopes changes? Custom Applications relying on this **undocumented and implicit** behavior will stop working.
...
Migrating your existing Project-level Custom Applications to the new Org-level Custom Applications is fairly straight-forward but it requires first to understand the new concepts like [OAuth Scopes and User Permissions](#oauth-scopes-and-user-permissions).
...
Upgrade your Application Kit packages (`@commercetools-frontend/*`) to version `>= 21`. Follow the instructions in the [Release notes](/releases/2022-01-25-custom-applications-v21).
...
The `entryPointUriPath` plays a crucial role in a Custom Application. Effectively, it's the **identifier** of the Custom Application similarly to the `projectKey` of the commercetools Platform.

<Info>

From now on, with the introduction of Org-level Custom Applications, the `entryPointUriPath` is **considered unique** within each [cloud region](https://docs.commercetools.com/api/general-concepts#regions) environment, meaning that there can't be any duplicates.
...
Furthermore, the `entryPointUriPath` value is bound to User Permissions of the Custom Application and is used to derive the unique name of the User Permissions.<br />
...
To make use of the [OAuth Scopes and User Permissions](#oauth-scopes-and-user-permissions) in local development, even though the Custom Application hasn't been configured in the Merchant Center yet, the login workflow works a bit differently from before.
...
We recommend informing your company and colleagues that are using the Custom Application about the migration and possible unavailability of it during the time of the switch.
...
At this point we recommend deactivating and removing the "old" Custom Application. You can then proceed with the deployment of the new setup.
