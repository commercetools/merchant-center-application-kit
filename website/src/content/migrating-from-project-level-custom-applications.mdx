...
* `icon`: The icon of the Custom Application. The value should be the SVG code of the icon itself. You can choose [one of the available icons](https://github.com/commercetools/merchant-center-application-kit/tree/main/packages/assets/application-icons) by referencing the path to that file using `${path:}`.
