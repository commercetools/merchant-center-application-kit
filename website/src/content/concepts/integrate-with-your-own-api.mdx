...
# Local development of a Custom Application with external API

Developing a Custom Application locally, together with an external API, has some limitations.
For example, using the `forward-to` endpoint of the Merchant Center API Gateway with a target URL pointing to `http` is not allowed. Only `https` targets are allowed.
Furthermore, the Merchant Center API Gateway cannot proxy a request to `localhost`.

The most straighforward solution is using the production URL of the external API, which may be inconvenient in the development processs.

Therefore, some kind of local tunnel solution can be used to develop both Custom Application and external API together, locally.

## ngrok

One of possible solutions is using the [ngrok](https://ngrok.com/) service. `ngrok` is a reverse proxy allowing to establish an encrypted tunnel between the `ngrok` agent application running locally and the upstream service.
...
can be further passed to the session middleware verifying the JWT audience claim.
...
To verify the tunneling service, configure the additonal environment variable `echoServerApiUrl` in your Custom Application:
```js title="custom-application-config.mjs" highlightLines="4"
const config = {
  // ...
  additionalEnv: {
    echoServerApiUrl: "https://33ea-87-183-162-9.eu.ngrok.io",
  },
```
and use it in requests sent to the `/proxy/forward-to` endpoint:

```jsx title="echo-server.jsx" highlightLines="8-10,12,19"
...
The entire implementation can be found under the following [link](https://github.com/commercetools/merchant-center-application-kit/blob/main/playground/src/components/echo-server/echo-server.jsx)

</Info>

In result, HTTP POST requests with a `x-forward-to` header, for example:
```
x-forward-to: https://33ea-87-183-162-9.eu.ngrok.io/?hello=world
```
...
The details of all requests tunneled via `ngrok` can be investigated in the web application running at `http://localhost:4040/`
