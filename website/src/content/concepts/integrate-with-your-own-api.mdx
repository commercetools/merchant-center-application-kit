...
- `getRequestUrl` (_function_): Returns the URL (URI path + query string) of the `request` to validate the audience. Use this in case the `request` object does not contain either a property `originalUrl` or `url` (see serverless example below for using AWS Lambda functions).
