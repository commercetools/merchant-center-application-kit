...
must be provided to the session middleware verifying the JWT audience claim.
...
In your Custom Application, we recommend configuring the external API URL through an environment variable, referenced in the application config through a [variable placeholder](/api-reference/application-config#using-variable-placeholders).
That would allow to keep the value for the development environment and for the production environment separated.

In the development environment configured with the external API URL exposed from `ngrok`, HTTP POST requests to `/proxy/forward-to` including the header:

```
x-forward-to: https://33ea-87-183-162-9.eu.ngrok.io/echo
```
...
`ngrok` agent application starts along with a monitoring web application running at `http://localhost:4040/`, which can be helpful in the process of debugging of the tunnelled HTTP requests.
