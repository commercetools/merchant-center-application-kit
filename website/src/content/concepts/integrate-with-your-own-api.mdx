...
* **permissions**: Optionally includes the list of applied permissions for the logged in user. The permission values are formatted as `can<permission-name>`, for example `canViewAvengers`.
...
Learn more about using this header in the helpers sections below.
...
- `<iss>/claims/user_permissions` (optional): This value contains a list of permissions names that are applied to the logged-in user. See [Configuring custom claims](#configuring-custom-claims). The claim is a **public claim** prefixed by a collision-resistant namespace, as per the JWT specification.
