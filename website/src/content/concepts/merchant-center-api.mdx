...
In local development the authentication is handled a bit differently using a OIDC login workflow. The session token is stored in `sessionStorage` and sent to the Merchant Center API via `Authentication` HTTP header.
