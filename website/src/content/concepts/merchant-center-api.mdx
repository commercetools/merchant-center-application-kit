...
| --- | --- |
| Australia (Google Cloud, Sydney) `gcp-au` | `mc-api.australia-southeast1.gcp.commercetools.com` |
| Europe (Google Cloud, Belgium) `gcp-eu` | `mc-api.europe-west1.gcp.commercetools.com` |
| Europe (AWS, Frankfurt) `aws-fra` | `mc-api.eu-central-1.aws.commercetools.com` |
| North America (Google Cloud, Iowa) `gcp-us` | `mc-api.us-central1.gcp.commercetools.com` |
| North America (AWS, Ohio) `aws-ohio` | `mc-api.us-east-2.aws.commercetools.com` |

# Authentication

The Merchant Center API is protected by a **session token** via the HTTP `Cookie` header.
...
There are other proxy endpoints available but they are mostly for internal usage and not documented.
