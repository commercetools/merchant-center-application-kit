...
However, in cases where more granular access control is needed, for example, only one team accross the organization is allowed to manage discount codes, the default permission pair is not enough.
