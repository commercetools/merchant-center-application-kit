...
## Custom Application local development

To start developing your Custom Application locally, the quickest way is by being part of the `Administrators` Team of your Organization. In that case, choose a Project that is part of an Organization where you are an administrator of.

<Info>

Starting from version `22.5.0`, an alternative option is provided for local development and login. This applies specifically to users with granular permissions assigned. Detailed information about this feature is outlined in the [help needed documentation](/help-needed#granular-permissions-for-custom-application-local-development)

</Info>

To enable local development of a Custom Application by a user being a member of a team with limited permissions go to [Application Config](/api-reference/application-config#envdevelopmentteamid-and-envdevelopmentapplicationid).
