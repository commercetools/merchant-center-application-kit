...
commercetools strives to improve tooling and packages around custom applications continously. This means that we do not follow a strict release cycle but instead publish releases in meaningful intervals, potentially multiple times a week depending on the version increment. In that patch and minor versions will be released more frequently than major versions.
...
- A dependency package drops support for an "old" Node.js version, thus releasing a major version. As a consequence, commercetools could release a new major version as well for dropping support to a specific Node.js version. However, this will unlikely affect consumers of custom application packages.
...
Some packages, for example `@commercetools-frontend/application-shell`, interact with internal commercetools HTTP APIs. Interactions with these APIs is encapsulated within these modules themselves to ensure backwards compatibility. However, changes to these modules may be included in rare updates of a package. In the process some HTTP API functionalities might be **deprecated** over time and **eventually removed**.
