...
- Compiling the `index.html.template` into `index.html` to inject the runtime configuration specified in the [application config](/development/application-config).
- Preparing the HTTP response headers, including the [custom headers](/development/application-config#headers) provided in the application config.
