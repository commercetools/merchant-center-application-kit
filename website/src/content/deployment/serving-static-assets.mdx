...
First, define the location URL of the storage directory in the `env.production.cdnUrl` in the [application config](/development/application-config).

For example, Merchant Center static assets are uploaded to a Google Cloud Storage bucket named `mc-production-eu` for the `eu` region's production assets.
...
To serve static assets on a remote CDN or Cloud Storage, do the following:

- Create a [production build](/deployment/production-build) of your Custom Application.
- [Compile](/deployment/compile-custom-application) your Custom Application.
- Upload the assets from `dist/assets` directory to a CDN or Cloud Storage.
...
To serve static assets using the HTTP server, do the following:
...
- Omit the `env.production.cdnUrl` property in the [application config](/development/application-config) file (the `env.production.url` is used instead).
