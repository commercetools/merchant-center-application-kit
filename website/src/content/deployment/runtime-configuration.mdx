...
Any envrionment configuration JSON file can contain references for environment variables. This can be useful to avoid duplication between various `env.json` files for multiple different environments. References are specified with an expansion like syntax `${}` while additionally prefixing the environment variable name with `env:`.

Imagine needing various `env.json` files which differ only in the `MC_API_URL` for three deployments of the Custom Application. We can use the `${env:MC_API_URL}` reference within the `env.json` to avoid needing various environment configuration files.
...
The `MC_API_URL` reference can then be filled in various ways. For instance with a run script in a `package.json` as `MC_API_URL=https://mc-api.commercetools.com yarn start` or via a [dotenv](https://github.com/motdotla/dotenv)-file which is sourced before starting the application.
