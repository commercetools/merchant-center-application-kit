...
`"@commercetools-frontend/actions-global"` package exposes both action creators (for instance `showNotification`) and hooks (for instance `useShowNotification`) that invoke the corresponding action creators under the hood.
