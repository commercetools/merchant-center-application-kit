...
Takes as an argument an object with `errors` property being an error object or an array of error objects.

Some sources of errors (for instance GraphQL APIs) return an array or object. `showApiErrorNotification` action creator accepts either as the value of `errors` property. 
...
In case errors array contains multiple error objects, the displayed notification will contain all of the error messages.
