...
The configuration for additional OAuth Scopes and user permissions extending [oAuthScopes configuration](#oauthscopes).
...
allows to send the additional permission information as claims scoped to each permission group:

```
view/movies:view_products view/merch:view_channels
```
...
```
manage/movies:manage_products manage/merch:manage_channels
```
