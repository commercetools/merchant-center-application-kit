...
A Custom Application requires some minimal configuration to be able to work, for example which [Cloud Region](/concepts/merchant-center-api#cloud-regions) to use, the identifier of the application, the required [OAuth Scopes](/concepts/oauth-scopes-and-user-permissions), and so on.

The configuration of a Custom Application is defined in a JSON file `custom-application-config.json` in the root of your project directory (next to `package.json`).
...
When using JSON files, you can take advantage of the [JSON schema for the application config](https://docs.commercetools.com/custom-applications/schema.json). To enable JSON schema autocompletion and validation support, add a reference to the `schema.json` URL in the VSCode settings (either user settings or workspace settings):
...
To learn more about the application config properties, read the following documentation.
...
The **unique identifier** of the Custom Application, similarly to the `projectKey` of the commercetools platform. This value determines the route at which the Custom Application is served by the Merchant Center.
...
Furthermore, the `entryPointUriPath` value is tightly bound to User Permissions of the Custom Application and is used to [derive the unique name of the User Permissions](/concepts/oauth-scopes-and-user-permissions#user-permissions).

Finally, make sure that your Custom Application routes also include the `entryPointUriPath`.
...
The identifier of the [Cloud Region](/concepts/merchant-center-api#cloud-regions) that the Custom Application uses to connect to the commercetools platform. The values map to the actual Merchant Center API URL for that region.
...
An object holding environment specific configurations, for example `development`, or `production`. Which environment is used depends on the environment variable `MC_APP_ENV`. If `MC_APP_ENV` isn't set then `NODE_ENV` will be used. If neither is set it defaults to `development`.
...
A `projectKey` of a project you have access to be used as the default project upon login in development.
...
The full URL value where the Custom Application is hosted at. This value is also used to configure the [Content Security Policy (CSP)](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP) HTTP headers to allow the browser to request data from the application domain.
...
The full URL value where the Custom Application [static assets](/deployment/serving-static-assets) are hosted, for example when using an external CDN.
...
The configuration for [OAuth Scopes and User Permissions](#oauth-scopes-and-user-permissions).

<Info>

You can have "view" or "manage" only OAuth Scopes and leave the other list field empty, as long as at least one OAuth Scope is specified.
...
A list of "manage-only" [OAuth Scopes](https://docs.commercetools.com/api/scopes) required by the Custom Application and associated to the "Manage" permission.
...
The visual identifier of the Custom Application in the Merchant Center. You can choose one of the following pre-defined icons by referencing it with the special expansion-like syntax `${path:}`:
...
An object describing the main link to the Custom Application in the Merchant Center left-side navbar menu.
...
A list of the [User Permissions](/concepts/oauth-scopes-and-user-permissions#user-permissions) specific to your Custom Application. Users must have at least one permission to see the link in the Merchant Center menu.
...
The values of the User Permissions for the Custom Application are always derived from the `entryPointUriPath`. For example given `entryPointUriPath: avengers`, the User Permissions are `ViewAvengers` and `ManageAvengers`.

</Info>

## `submenuLinks`

An list of objects describing the sub-links to the Custom Application in the Merchant Center left-side navbar menu.
...
You can use a relative path or a path to a module.
...
Many of the information specified in the application config is injected into the runtime environment and available via the global variable `window.app`.
