...
Please make sure to follow the guidelines of Custom Application [deployment](/deployment-examples) and [configuration in the Merchant Center](/merchant-center/managing-custom-applications). After Custom Application installation and assigning team permissions, provide values for `env.development.applicationId` and `env.development.teamId` respectively.
