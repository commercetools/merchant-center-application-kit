...
In general the `test-utils` simulate the components-under-test as if it was rendered by the `<ApplicationShell>` and provides the necessary setup to fully test a Custom Application. This includes things like Apollo, React Intl, React Router, etc.

## renderApp

A wrapper around the [`render`](https://testing-library.com/docs/react-testing-library) method of React Testing Library. All the basic setup for testing is included here.
