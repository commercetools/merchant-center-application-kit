...
By default, compiling the `index.html` is implicitly done in the `build` command, unless the option `--build-only` is used. In that case, you need execute the `compile-html` command separately.
