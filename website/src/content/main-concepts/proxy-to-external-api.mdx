...
These decoded JWT token consist of **Register Claim Names** and **Private claims** and have the following fields:


- `exp`: This claim identifies the **expiration time**. The value of this claim requires to be the time before the current time to be valid and accepted.
- `sub`: This claim identifies the **subject**. The value of `sub` issued by the Merchant Center API Gateway for Custom Applications is the user-ID for Merchant Center.
- `iss`: This claim identifies the **issuer**. The value of `iss` is either a cloud identifier or a valid URL to the Merchant Center API Gateway. The value differs depending on the region as outlined above.
- `aud`: This claim identifies the **audience**. The value of **audience** is the exact URL to the External API.
- `type`: This is public claim identified by the Merchant Center API Gateway. The decoded JWT in the example above is a result of authorization exchange between Merchant Center API Gateway and the External API, hence the name.
- `<hostname>/claims/project_key`: This is a claim identified by the Merchant Center API Gateway. This identifies the project key that the JWT was issued for.
