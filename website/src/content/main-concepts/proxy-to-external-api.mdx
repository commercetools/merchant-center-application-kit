...
To facilitate the usage of the [built-in Apollo client and the SDK actions](/main-concepts/data-fetching), we provide some utility helpers to abstract away the request configuration for the HTTP headers listed above.
...
It's imperative that the external API is securely protected and that only users with access to the project are able to connect to the API.
...
Finally, you can use the middleware as following:
...
The Merchant Center API Gateway exposes a `/.well-known/jwks.json` endpoint that can be used to validate JWTs, as per Open ID Connect (OIDC) spec. The endpoint provides a rotating public key used to verify the JWT signature.
...
If you use the `@commercetools-backend/express` package, you don't need to do anything in this regard.
...
The following table lists all the supported versions and their deprecation period:
