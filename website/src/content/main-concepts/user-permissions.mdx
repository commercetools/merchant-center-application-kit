...
A React component that evaluates the demanded permissions against the user permissions and renders the result as a function with the a named argument `isAuthorized`.
...
It is recommended to store general permissions and action rights used by your application into a `permissions.js` file. This helps to avoid repeating permission names across your code base. Preventing typographical errors while giving an overview of used permissions in an application easily.
