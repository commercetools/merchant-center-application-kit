...
To handle requests to the [GraphQL APIs](/main-concepts/graphql) we use the [Apollo GraphQL Client](https://www.apollographql.com/docs/react/), which comes already configured within the [`<ApplicationShell>`](/main-concepts/application-shell).
...
Some endpoints or APIs might not be available as GraphQL, which means that we need to fetch the data using a normal HTTP REST endpoint.
There are different options here: you can build and send requests manually, using for example the [`fetch`](https://github.github.io/fetch/) library or any HTTP client.

However, we recommend using our own declarative fetching library [`@commercetools-frontend/sdk`](https://www.npmjs.com/package/@commercetools-frontend/sdk), which builds on top of the [JS SDK client](https://commercetools.github.io/nodejs/sdk/api/sdkClient.html) and [Redux](https://redux.js.org/) and is already configured within the [`<ApplicationShell>`](/main-concepts/application-shell).
...
Ideally, you would want to handle data normalization, caching etc. The data should also be lifted up to the global Redux store, which allows other components to access the same data afterwards.
