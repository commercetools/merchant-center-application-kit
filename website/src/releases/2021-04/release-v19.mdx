...
In this version there are no explicit changes in the packages but mainly updates to major versions of some of the dependency libraries. For instance:
...
For the `<Constraints.Horizontal />` component, the deprecated `constraint` prop has been removed. Now only the `max` prop is used instead.
...
For the `<Text.Body>` and `<Text.Detail>` components, the deprecated `isInline` prop has been removed. Now only the `as` prop is used instead.
...
For the `<Tag>` component, the deprecated `linkTo` prop has been removed. Now only the `to` prop is used instead.

## SecondaryButton

For the `<SecondaryButton>` component, the deprecated `linkTo` prop has been removed. Now only the `to` prop is used instead.
