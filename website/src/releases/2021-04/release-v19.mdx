...
For the `<Headline>` and `<Subheadline>` components, the deprecated `elementType` prop has been removed. Now only the `as` prop is used instead.
...
For the `<Body>` and `<Detail>` components, the deprecated `isInline` prop has been removed. Now only the `as` prop can be used instead.
