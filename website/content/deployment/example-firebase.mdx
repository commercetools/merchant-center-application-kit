...
title: Example Firebase - Deployment
...
4. Choose to use ESLint and install dependencies with npm (both recommended). It is safe to decline to manage dependencies in another way
...
You can set up one or more Firebase Hosting sites in a single Firebase project. This is useful to host multiple custom applications on a single Firebase/GCP project.
...
You also need a `csp.json` file. You can also have a `csp.prod.json` file to use a different configuration for `development`.
...
To generate the correct configuration for Firebase, the `mc-scripts compile-html` script must use a Firebase transformer. This transformer will generate a `firebase.json` file with the custom security headers required for Merchant Center Custom Applications.
