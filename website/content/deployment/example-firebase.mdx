...
3. Select the default directory to use as your public root directory. The default is `public` and is where the compiled files will be placed.

4. Configure your site as a single-page app to automatically add rewrite configurations.
...
## _(Optional)_ Set up a Hosting Site
...
module.exports = ({ headers }) => {
  ...
  const config = target ? { ...baseConfig, target } : baseConfig;
...
If you go to `https://[projectID].firebaseapp.com`, you will see the application in an authorization redirection loop. **This is expected** as the application is not being loaded behind the Merchant Center Proxy.
