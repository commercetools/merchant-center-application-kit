...
beforeAll(() => {
  ...
  jest.spyOn(console, 'error').mockImplementation(jest.fn()); //   jest.spyOn(console, 'error').mockImplementation(jest.fn()); // This is to silence logs raised by the tests where we mock a response error from the server
});
afterAll(() => {
  mockServer.close();
