...
/**
 * Converts a URI path into a breadcrumb-like string.
 * Examples:
 *
 ...
 *   /login/new => login > new - Merchant Center
 */
const compilePageTitle = <Query extends {}>(
  location: TEnhancedLocation<Query>
): Breadcrumb<Query> => {
  const [, projectKeyOrStaticPath, ...locationPaths] = location.pathname.split(
    '/'
  );
...
const ApplicationHelmet = <Query extends {} = {}>(props: Props<Query>) => {
...
ApplicationHelmet.displayName = 'ApplicationHelmet';
