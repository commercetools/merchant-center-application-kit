...
- **Visual coverage lives elsewhere, on two surfaces**: most of the repo's stories cover this package, but they live in `storybook/src/stories/` and import through the public entrypoint. The Cypress playground snapshots these components too, in context. Changing a component here can change a Chromatic snapshot in either project with nothing in this directory to hint at it. See `storybook/AGENTS.md` and `docs/chromatic-components.md` for the Storybook surface, `docs/chromatic-e2e-playground.md` for the playground.
