...
const templatePackageName = `@commercetools-frontend/application-template-${templateName}`;
const modulePathForPackage = require.resolve
  .paths(templatePackageName)
  .find(modulesPath =>
    fs.existsSync(path.join(modulesPath, templatePackageName))
  );
const templatePath = path.join(modulePathForPackage, templatePackageName);
