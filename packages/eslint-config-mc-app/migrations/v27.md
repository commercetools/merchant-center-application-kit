...
- **New lint violations** — the dependency upgrades may flag new issues:
  ...
  - `eslint-plugin-react-hooks` v4→v5 (improved detection); rules now also apply to `*.ts` files — custom hooks in `.ts` files may surface new `react-hooks/exhaustive-deps` warnings
...
The base config registers plugins only for the file types where their rules apply (e.g. `@typescript-eslint` for `*.ts`/`*.tsx`, `react-hooks` for `*.ts`/`*.tsx`/`*.js`/`*.jsx`). Common cases:

- A `.js` file with `// eslint-disable-next-line @typescript-eslint/...`
- A `.cy.ts` file with `// eslint-disable-next-line jest/...`
