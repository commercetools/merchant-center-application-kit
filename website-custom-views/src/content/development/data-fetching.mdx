...
A Custom View uses the [Merchant Center API](/concepts/merchant-center-api) to connect to any of the Composable Commerce APIs.
...
Custom Views provide a pre-configured [Apollo GraphQL Client](https://www.apollographql.com/docs/react/) to perform requests to the GraphQL APIs.
...
In Custom Views you define your queries and mutations in dedicated `.graphql` files that you can `import` in your component file.
...
Custom Views provide a pre-configured declarative fetching library [`@commercetools-frontend/sdk`](https://www.npmjs.com/package/@commercetools-frontend/sdk), which builds on top of the [JS SDK client](https://commercetools.github.io/nodejs/sdk/api/sdkClient.html) and [Redux](https://redux.js.org/)
...
By default Custom Views provide pre-configured HTTP clients for [GraphQL](#graphql-http-client) and [REST](#rest-http-client) API requests.
...
In case your Custom View needs to connect to an [external API](/concepts/integrate-with-your-own-api), in addition to the Composable Commerce APIs, the HTTP client has to be reconfigured to connect to the `/proxy/forward-to` endpoint with the appropriate headers.
...
## Fetching host application view related data

You can use Custom Views to display and manage any kind of data you might need. There can be use cases where the data you want to use in your Custom View is dependant on the entity the underlying host application view is displaying.

In such situations, you can use the [hostUrl](/api-reference/commercetools-frontend-application-shell-connectors#hosturl) property from the [Custom View context](/api-reference/commercetools-frontend-application-shell-connectors#custom-view-context) to get the URL of the host application and extract the ID of the displayed entity.

For example, if the Custom View is rendered in the Order details view of the Orders application and you want to load some data related to the specific order which is being displayed in the host application.
You can access the host application URL using the [useCustomViewContext](/api-reference/commercetools-frontend-application-shell-connectors#usecustomviewcontext) hook and extract the order ID from the URL.

```jsx title="my-custom-view.jsx"
...
const MyCustomView = () => {
  ...
  // The order details view URL looks like this:
  // https://<merchant-center-domain>/<project-key>/orders/111111-2222-3333-444-5555555555/general
  const url = new URL(hostUrl);
  const pathnameParts = url.pathname.split('/');
  const orderId = pathnameParts[3]; // 111111-2222-3333-444-5555555555

  return (
    <div>
      <h1>Current Order ID: {orderId}</h1>
    </div>
  );
}
