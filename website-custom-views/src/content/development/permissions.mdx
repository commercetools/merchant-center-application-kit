...
In the Custom View config, the menu links allow defining the level of required access by providing the name of the requested user permission.
...
In case you defined the permissions in a `constants` file, we recommend to reference them in the Custom View config as well. To do so, the config file needs to be converted to a JS file (instead of JSON), for example using the `.mjs` file extension.
