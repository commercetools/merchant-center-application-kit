...
This should give you a basic idea on how you can approach testing your Custom View.
...
By default, the [`test-utils`](/api-reference/commercetools-frontend-application-shell#test-utils) assigns the pre-defined permissions for the user to be able to view the Custom View. If you need to test with more permissions, you need to explicitly provide them in your test setup. The following field can be used to assign the different granular permission values:

* `projectAllAppliedPermissions`: a list of applied resource permissions that the user should have for the given Project. A resource permission is an object with the following shape:
...
In the `cypress/plugins/index.js` file, you need to configure the `task` for Custom Views and to load the environment variables:
