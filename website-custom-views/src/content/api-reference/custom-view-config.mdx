...
```json
{
  "name": "The favourite Products view"
}
```
...
```json
{
  "description": "Manage your favourite Products"
}
```
...
{
  "labelAllLocales": [
    { "locale": "en", "value": "The Products" },
    { "locale": "de", "value": "Die Products" }
  ]
}
```
...
To make the Custom View config more reusable across different environments, you can use references to environment variables within the config file.
...
References for Intl messages are specified with a special expansion-like syntax `${}` together with the prefix key `intl:`. For example, to load a translation message from the `en.json` file named `Menu.Products`, you would use `${intl:en:Menu.Products}`.
...
{
  "labelAllLocales": [
    { "locale": "en", "value": "${intl:en:Menu.Products}" },
    { "locale": "de", "value": "${intl:de:Menu.Products}" }
  ]
}
```
