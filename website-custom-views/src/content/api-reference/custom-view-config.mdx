...
The Custom View [context](/api-reference/commercetools-frontend-application-shell-connectors#custom-view-context) will contain the ["hostUrl"](https://docs-beta-custom-views.commercetools.vercel.app/custom-views/api-reference/commercetools-frontend-application-shell-connectors#hosturl) property which holds the value of the host built-in application view where the user opens it.

When working locally, by default you will see the `hostUrl` value pointing to local enviroment: `http://localhost:3001/<projectKey>/@@custom-view-host@@`.

You can override this default behaviour to set a custom URI path so you can test the Custom View in the context of a specific built-in application view.

```json
{
  "env": {
    "development": {
      "hostUriPath": "/my-project/products/111-2222-33333-44444"
    }
  }
}
```
