...
Generic components for building Merchant Center Custom Views.
...
Custom Views typically consist of a variety of pages with different hierarchical levels. For example, a page to show a list of products, followed by a detail page to see the product's details.
...
The following page components should be used in the landing pages of the Custom View that don't have a parent page in the hierarchy. For example, for a Custom View for products, the main page can show a list of products.
...
The `<TabularMainPage>` is supposed to be used in one of the main application landing pages, as the top-level component page. The page should have no parent pages in the hierarchy.
...
The following page components should be used in the pages of the Custom View that show data of a specific resource. Hierarchically, the parent page should be the main page. For example, for a Custom View for products, the detail page shows the product's data.
...
The `<TabularDetailPage>` should be used as a direct child of one of the main pages. Typically it's used as a detail page with multiple sub-pages (tabs).
...
The following page components should be used in nested pages of the Custom View. Hierarchically, the parent page should be the detail page. For example, for a Custom View for products, the modal page shows the product's variants data.
...
The following page components can be used on any page of the Custom View. For example, showing a confirmation dialog before performing an important action, or showing additional information as an info box.
...
When displaying the content of a page, it's important to structure it properly to establish a visual hierarchy and provide the users with a familiar user experience. To learn more about different layouts, see [Page Content Layouts](/development/page-content-layouts).

We provide the following components to help you create page content layouts:

## PageContentNarrow

Renders the children in a [narrow page content layout](/development/page-content-layouts#narrow).
...
Renders the children in a [wide page content layout](/development/page-content-layouts#wide-single-column).
...
| Props | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `columns` | `enum` <br/><br/>`1`, `1/1`, `2/1` | - | `1` | For a single-column layout, use the `1`. For a two-column layout with equal widths, use `1/1`.  For a two-column layout with column widths in a 2:1 ratio, use `2/1`. |
| `gapSize` | `enum` <br/><br/>`10`, `20` | - | `20` | The space between the two columns. |
| `children` | `node` | ✅ | - | Content rendered within this layout. |
...
Renders the children in a [full page content layout](/development/page-content-layouts#wide-single-column).
...
# Usage outside of Custom Views

If you happen to use some of the components outside of a Custom View, you need to additionally render the `<PortalsContainer>`.
