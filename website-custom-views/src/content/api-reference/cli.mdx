...
The CLI of Custom Views allows you to start and build your Custom View.
...
This command compiles the `index.html.template` file into an `index.html` file according to the [Custom View config](/api-reference/application-config). The production assets are then copied to the `public` folder and are ready to be deployed.
...
This command starts a HTTP server to serve the previously built and compiled Custom View, from the `public` folder. This is useful for testing the production build locally.
...
The command uses the [cloud Region](/concepts/merchant-center-api#cloud-regions) environment information from the Custom View config. The API tokens are stored separately for each cloud Region.
...
This command allows users to synchronize the local Custom View config with the Merchant Center. The sync implies that a new Custom View will be created or an existing one will be updated.

Developers can use the `config:sync` command to automatically manage the configuration of a Custom View from the config file instead of having to manually fill out the information in the Merchant Center.
...
Additionally, after creating a new Custom View using the `config:sync` command, you must add the generated [Custom View ID](/api-reference/application-config#envproductionapplicationid) to the application config file, following the instructions in the terminal.
...
The command uses the [cloud Region](/concepts/merchant-center-api#cloud-regions) environment information from the Custom View config.
