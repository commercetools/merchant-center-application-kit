...
**func** *(optional)*\
The render function is called when the `<CustomViewShell>` is ready to render the actual application. This is the case when the required data (user, project) has been fetched and the application context has been initialized.<br/>It's **recommended to use** the `children` prop to benefit from a simpler setup.
...
**boolean**\
Pass `true` if you are using the `render` prop for the `<CustomViewShell>` instead of the `children` prop.
