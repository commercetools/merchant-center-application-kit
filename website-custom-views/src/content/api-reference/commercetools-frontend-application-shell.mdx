...
The `CustomViewShell` component must be rendered as the top-level component of your application.

### Usage

We recommend to render the application content as `children` of `<CustomViewShell>`.

This lets the `<CustomViewShell>` pre-configure the application entry point routes. In addition to that, the entry point route is protected by the basic `View` permission check. This means that a user without permissions of your Custom View won't be able to access the Custom View route.
