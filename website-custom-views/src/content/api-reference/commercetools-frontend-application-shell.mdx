...
The `<CustomViewShell>` pre-configures the Custom View entry point routes. In addition to that, the entry point route is protected by the basic View permission check. This means that a user without permission from your Custom View won't be able to access the Custom View route.
...
**string**\
Defined the URL the Custom View will receive as part of the emulation of it being rendered in a Merchant Center built-in application.
