...
The name of the Custom View, for information purposes.
...
The **unique identifier** of the Custom View, similarly to the `projectKey` of the Composable Commerce APIs. This value determines the route at which the Custom View is [served by the Merchant Center Proxy](/concepts/merchant-center-proxy-router).
...
Furthermore, the `entryPointUriPath` value is bound to user permissions of the Custom View and is used to [derive the unique name of the user permissions](/concepts/oauth-scopes-and-user-permissions#user-permissions).
...
Even though the `entryPointUriPath` is a unique identifier of a Custom View, and thus should be defined statically in the source code, it can also be defined dynamically via environment variables using the [environment placeholders](#using-variable-placeholders).

This may be the case if the Custom View is to be deployed in different environments.
...
The visual identifier of the Custom View in the Merchant Center. You can choose one of the following predefined icons by referencing it with the special expansion-like syntax `${path:}`:
...
An object describing the main link to the Custom View in the Merchant Center main navigation.
...
The values of the user permissions for the Custom View are always derived from the `entryPointUriPath`. See [Applying user permissions](/development/permissions#applying-user-permissions) for more information.

</Info>

## `submenuLinks`

A list of objects describing the sub-links to the Custom View in the Merchant Center main navigation.

## `submenuLinks.*.uriPath`

The URI path relative to the `entryPointUriPath` of the Custom View.<br />
...
This is useful when specifying the [`icon`](#icon) of the Custom View.
