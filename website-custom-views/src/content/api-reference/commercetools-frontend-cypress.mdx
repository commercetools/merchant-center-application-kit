...
This command performs the user login to the Merchant Center, designed specifically for testing a Custom View. Testing Custom Views is expected to occur on localhost, therefore the `cy.loginByOidc` mechanism is used.<br/>
...
Ensure that the `PROJECT_KEY` environment variable is available if the related `projectKey` option isn't provided explicitly.
